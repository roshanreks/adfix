        ...
        <FadeIn delay={0.1} className="mt-10">
          <div className="flex flex-wrap gap-3">
            {pills.map((p) => (
              <div
                key={p.label}
                className="inline-flex items-center gap-2 rounded-full border border-[#E2E8F0] bg-white px-4 py-2 text-[14px] font-medium text-[#475569]"
              >
                <p.icon className="h-4 w-4 text-[#6D28D9]" />
                {p.label}
              </div>
            ))}
          </div>
        ...
        <div className="mt-16 grid grid-cols-1 gap-6 lg:grid-cols-3">
          {objections.map((o) => (
            <FadeIn key={o.quote} delay={0.15}>
              <div className="flex h-full flex-col rounded-xl border border-[#E2E8F0] bg-white p-8 transition-all duration-200 hover:border-[#C4B5FD] hover:shadow-ambient-hover hover:-translate-y-0.5">
                <span className="font-serif text-[48px] leading-none text-[#F3E8FF]">&ldquo;</span>
                <h3 className="text-[18px] font-medium text-[#0F172A]">{o.quote}</h3>
                <p className="mt-3 text-[16px] leading-[1.6] text-[#475569]">{o.answer}</p>
              </div>
            </FadeIn>
          ))}
        </div>
